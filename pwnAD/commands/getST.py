...
from impacket.krb5.crypto import Key, _enctype_table, _HMACMD5, _AES256CTS, Enctype, _get_checksum_profile, Cksumtype
...
class GETST:
    ...
    def doS4U(self, tgt, cipher, oldSessionKey, sessionKey, nthash, aesKey, kdcHost):
        ...
        if self.__dmsa:
            ...
            paencoded = encoder.encode(paForUserEnc)
...
def getST(auth):
    ...
    if (auth.cert and auth.key) is not None:
        ...
        try:
            auth.nthash = getNThash(auth)
        except Exception as e:
            logging.error(f"Error while trying to recover NThash : {e}")
    try:
        getst = GETST(auth.username, auth.password, auth.domain, auth)
        getst.run()
    except Exception as e:
